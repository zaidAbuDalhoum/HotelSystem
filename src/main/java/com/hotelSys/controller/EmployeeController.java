...
public class EmployeeController {
    ...
    public ModelAndView getEmployeeHomePage(HttpServletRequest request, HttpServletResponse response) {
    ...
    public ModelAndView showPendingOrders(HttpServletRequest request, HttpServletResponse response) {
        ...
        HttpSession session = request.getSession(false);
        List<Order> list = orderDaoEmployee.viewPendingOrders((String) session.getAttribute("permission"));
        if (list.isEmpty()) {
    ...
    public ModelAndView showComplaints(HttpServletRequest request, HttpServletResponse response) {
        ModelAndView mav ;
        HttpSession session = request.getSession(false);
        List<Complaint> list = complaintDaoEmployee.viewComplaintsForEmployee((String) session.getAttribute("permission"));
        if (list.isEmpty()) {
    ...
    public static ModelAndView checkEmployeePermission(String permission,ModelAndView modelAndView){

        if (permission.equalsIgnoreCase("restaurant") ||
        permission.equalsIgnoreCase("dryCleaning"))
            return modelAndView;
        else
            return new ViewsGenerator().getHomePage(permission);
