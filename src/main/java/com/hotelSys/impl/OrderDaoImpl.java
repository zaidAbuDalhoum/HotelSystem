...
public class OrderDaoImpl implements OrderDao {
  ...
  public void addOrder(final OrderWrapper orderWrapper, String type) {
    ...
    logger.info("a New Order was Placed : " + order.getBill());
  }
  ...
  public List<Order> viewAllOrders(final String type) {
    ...
    if (type.equalsIgnoreCase("all")) {
      ...
      return jdbcTemplate.query(SELECT, getOrderMapper());
  ...
  private static List<Double> calculateBill(List<Item> items) {
    double totalSum = 0;
  ...
  private static String generateBillDescription(List<Item> items) {
  ...
  private List<Item> getItemsById(final int id) {
  ...
  private RowMapper<Order> getOrderMapper() {
    ...
    RowMapper mapper = new RowMapper<Order>() {
      @Override
      public Order mapRow(ResultSet resultSet, int i) throws SQLException {
        Order order = new Order();
        order.setOrderId(resultSet.getInt("orderId"));
  ...
  private RowMapper<Item> getItemMapper() {
